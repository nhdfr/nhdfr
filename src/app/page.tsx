            ...
            {recentBlogs.map((post, i) => (
              ...
              >
                <span className="flex items-start gap-2 sm:gap-3 min-w-0 w-full">
                  <span
                    className="hidden sm:block w-6 shrink-0 tabular-nums text-foreground/60"
                    aria-hidden
                  >
                    {(i + 1).toString()}.
                  </span>
                  <span className="font-medium text-base whitespace-normal break-words leading-snug line-clamp-2 pr-2 flex items-center gap-2">
                    {post.title}
                    <span
                      className="opacity-0 translate-x-[-2px] group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200"
                      aria-hidden
                    >
                      →
                    </span>
                ...
                <span className="self-end sm:self-auto text-muted-foreground text-[11px] sm:text-sm whitespace-nowrap sm:min-w-[120px] sm:text-right sm:justify-self-end mt-0.5 sm:mt-0">
